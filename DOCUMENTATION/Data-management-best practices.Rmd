...
This R Notebook (.rmd) contains blocks of executable code along with detailed analysis and documentation of the code and workflow. To use this Notebook, render (knit) it as an HTML document into the RStudio Viewer Pane:
...
#### Core packages: `here`, `tidyverse`
###### EXECUTABLE CODE
```{r core, eval = FALSE}
suppressMessages(library(here))
suppressMessages(suppressWarnings(library(tidyverse)))
```
Use `base::library()` to load the core packages for file path specification (`here`) and data wrangling (`tidyverse`). We can use `base::suppressMessages()` and `base::suppressWarnings()` to quiet chatter in the console.

Using `here()` to specify file paths enables a robust workflow. `here()` anchors the file path to the top-level project folder. Thus, other users of the script need only replicate the standard project folder structure, and read/write functions will run without error.
