...
```
In the next sequence, we use `runner::fill_run()` to label rows (items) for recoding. Because we need `fill_run()` to operate only within values of `ID` (that is, within single cases), we call `dplyr::group_by()` to group the data object by `ID`.

We then invoke `mutate()` to create a new column `recode_run` to hold the start and end points for the item ranges to be recoded. We assign values using `case_when`, marking the start item for first range to be recoded with the expression `start1 == item ~ "recode1"`. When evaluated, this expression finds the row that represents the item named in `start1`; that is, the _first_ item in the first item range to be recoded. In this row, it assigns the value `recode1` to the column `recode_run`. With the expression `end1 == item ~ "recode1"`, `case_when()` assigns `recode1` to mark the row containing the _last_ item in the first item range to be recoded. It then repeats this process for the second and third ranges to be recoded, for any cases that have additional ranges. As a result, the start and end points for up to three item ranges per case (`ID`) are now marked.
```{r script, echo = 67:78, eval = F}
```
