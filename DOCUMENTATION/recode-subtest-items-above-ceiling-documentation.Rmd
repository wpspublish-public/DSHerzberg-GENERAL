...
The central operation here is counting consecutive `0` responses in the data, and recoding only items above the level where the ceiling rule is reached. To accomplish this, we first pivot the data from wide to long format, such that consecutive items are structured as consecutive rows, instead of consecutive columns. This allows the row-wise application of key `dplyr` and `tidyr` functions, and facilitates more concise code.
