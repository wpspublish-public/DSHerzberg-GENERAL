...
```
We then `pivot_longer()` to transform the `input` data object into the tall (multilevel) format. We use the `cols = -ID` argument to exclude the `ID` column from the pivot, so that in the resulting data object, item-reponse pairs are nested within each value of `ID`. `pivot_longer()` adds duplicate rows for each value of `ID`, such that in the transformed data object, there are _n_ rows containing each value of `ID`, where _n_ is equal to the number of item columns in the original wide-format `input` object.

As part of this transformation, `pivot_longer()` splits the names of the orginal item columns into two parts, and stores these parts in two columns in the long data object. The argument `names_to = c("pre", "num")` gives the names of the two destination columns, and the argument `names_sep = 3` specifies that the original names will be split after the third character. The argument `values_to = ` is omitted here, meaning the cell values from the original item columns will be stored in a destination column with the default name of `value`.
