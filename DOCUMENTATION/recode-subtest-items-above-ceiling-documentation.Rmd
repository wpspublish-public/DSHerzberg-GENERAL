...
input_long <- input %>%
  ...
  group_by(ID, pre) %>%
  mutate(
    streak_val = case_when(value == 0 ~ streak_run(value, na_rm = FALSE),
                           TRUE ~ NA_integer_),
    ceiling = case_when(
...
ceiling <- input_long %>% 
  group_by(ID, pre) %>% 
  summarize(ceiling_count = sum(ceiling)) %>% 
...
```{r script, echo = 19:27, eval = FALSE}
```
The next step of the process is to mark those subtests, across persons, where the stop rule is satisfied. We store these marks in a data frame `celing`, which we derive by first grouping `input_long` by `ID` and `pre` (subtests within persons). We then call `dplyr::summarize()` on the grouped data, 
```{r script, echo = 28:30, eval = FALSE}
```
